...
function getTimer($selector, $dateTime) {
  ...
  function showSec() {
    ...
    if (distance < 0) {
      ...
      var mseconds = '00';
      try {
        mseconds = Math.floor((distance % _minute) % 1000)
          .toString()
          .substring(0, 2);
      } catch (e) {}
      ...
      $(selector)
        .find('.day')
        .text(days.toString().length == 1 ? '0' + days : days);
      $(selector)
        .find('.hour')
        .text(hours.toString().length == 1 ? '0' + hours : hours);
      $(selector)
        .find('.min')
        .text(minutes.toString().length == 1 ? '0' + minutes : minutes);
      $(selector)
        .find('.sec')
        .text(seconds.toString().length == 1 ? '0' + seconds : seconds);
...
function introView() {
  ...
  var _nickname = ['Ted', 'Heather', 'Ken', 'Dave', 'Olivia', 'Sun', 'Alyssa', 'Fran'];
  var _position = ['Studio FLO', 'Studio FLO', '추천기술팀', '서버개발팀', '디스커버FLO기획팀', '크리에이티브팀', '플랫폼사업팀', '큐레이션팀'];
  var _title = [
